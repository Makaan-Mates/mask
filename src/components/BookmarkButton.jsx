...
const BookmarkButton = () => {
  ...
  const updateBookmark = async () => {
    ...
    const data = await fetch(`https://mask-backend.up.railway.app/api/user/bookmark/${postid}`, {
      method: "POST",
      headers: {
  ...
  const getUserDetails = async () => {
    const data = await fetch(`https://mask-backend.up.railway.app/api/post/bookmark/${postid}`);
    const json = await data.json();
    setPostDetails(json)
  ...
  return (
    <div className="flex gap-4 py-2 items-center text-[#9B9B9B]">
      <span className="flex items-center cursor-pointer">
        {isBookmarked ? (
          <FaBookmark onClick = {handleBookmarkClick} className="mr-2 text-lg text-[#9B9B9B] hover:text-[#d2d2d2]" />
        ) : (
            <FaRegBookmark onClick = {handleBookmarkClick} className="mr-2 text-lg text-[#9B9B9B] hover:text-[#d2d2d2]" />
        )}
        {}
      </span>
    </div>
